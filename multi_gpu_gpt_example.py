...
def main():
    ...
    parser.add_argument('--batch_info_file', type=str, default=None,
                        help='path to batch info file. It contains specific batch size for each batch.')
    parser.add_argument('--arrival_info_file', type=str, default=None,
                        help='path to arribal info file. It contians all request arrival timestamp in ms.')
    parser.add_argument('--request_input_file', type=str, default=None,
                        help='path to request input file. It contains token id of each request.')
    parser.add_argument('--output_length_file', type=str, default=None,
                        help='path to output length file. It contains required output length for each request.')
    ...
    # Inputs
    processed_inputs_num = 0
    total_inputs_num = 0
    inputs = []
    
    if args.request_input_file:
        with open(args.request_input_file, "r") as f:
            lines = f.read().splitlines()
            for line in lines:
                tokens = line.split()
                tokens = [int(token) for token in tokens]
                inputs.append(tokens)
            total_inputs_num = len(inputs)
    else:
    ...
    if args.batch_info_file:
        batch_size_list = []
        with open(args.batch_info_file, "r") as f:
            lines = f.read().splitlines()
            for bs in lines:
                batch_size_list.append(int(bs))
    
    arrival_list = []
    with open(args.arrival_info_file, "r") as f:
        lines = f.read().splitlines()
        for arrival in lines:
            arrival_list.append(float(arrival)/1000)
    iteration = 0
    
    token_generated_list = []
    with open(args.output_length_file, "r") as f:
        lines = f.read().splitlines()
        for length in lines:
            token_generated_list.append(int(length))
    ...
    # Ensure there is at least one input when starting inference.
    sleep(arrival_list[0])
    ...
    while True:
        ...
        end_batch_indx = processed_inputs_num
        while end_batch_indx < total_inputs_num \
            and end_batch_indx < processed_inputs_num + max_batch_size \
            and arrival_list[end_batch_indx] < timeit.default_timer() - inference_start:
                end_batch_indx += 1
        print("--------------------------------------------")
        print(f"Process inputs: {processed_inputs_num} - {end_batch_indx}")
        print(f"Start at {timeit.default_timer() - inference_start:.2f} seconds elapsed")
        
        if end_batch_indx == processed_inputs_num:
            print("No more inputs to process at this time.")
            continue
        
        batch_size = end_batch_indx - processed_inputs_num
        contexts = inputs[processed_inputs_num : end_batch_indx]
        output_len = max(token_generated_list[processed_inputs_num : end_batch_indx])
        processed_inputs_num = end_batch_indx
            
        # batch_size = min(total_inputs_num - processed_inputs_num, batch_size_list[iteration])
        # contexts = inputs[processed_inputs_num : processed_inputs_num + batch_size]
        # processed_inputs_num += batch_size
        
        start_ids = None
        if args.request_input_file:
            start_ids = [torch.tensor(c, dtype=torch.int32, device=device) for c in contexts]
        else:
        ...
        if iteration == 0:
            ...
            infer_decode_args['top_k'] = top_k * torch.ones(batch_size, dtype=torch.int32)
            infer_decode_args['top_p'] = top_p * torch.ones(batch_size, dtype=torch.float32)
            infer_decode_args['temperature'] = temperature * torch.ones(batch_size, dtype=torch.float32)
            infer_decode_args['beam_search_diversity_rate'] = beam_search_diversity_rate * torch.ones(batch_size, dtype=torch.float32)
            infer_decode_args['len_penalty'] = len_penalty * torch.ones(size=[batch_size], dtype=torch.float32)
            infer_decode_args['min_length'] = min_length * torch.ones(size=[batch_size], dtype=torch.int32)
        ...
        if args.time:
            ...
            print(f'[INFO] GPT time costs: {time_elapsed * 1000:.2f} ms')
        ...
        if rank == 0:
            ...
            for i, (context, tokens) in enumerate(zip(contexts, tokens_batch)):
                for beam_id in range(beam_width):
                    ...
                    print(f'[latency] {(finish_time - inference_start - arrival_list[processed_inputs_num - batch_size + i]):.6f} s')
                    print(processed_inputs_num - batch_size + i)
        ...
        if total_inputs_num <= processed_inputs_num:
            break
